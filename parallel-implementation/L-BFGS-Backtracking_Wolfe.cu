...
vector<double> LBFGS_CUDA(
    ...
    for (int k = 0; k < max_iterations; ++k) {
        ...
        *h_f_x = f(x_host);
        ...
        checkCublasError(cublasDdot(cublasHandle, size, d_g, 1, d_d, 1, h_grad_dot_d),
                        "CUBLAS ddot for gradient dot direction failed");
        
        double alpha_current = INITIAL_STEP_SIZE;
        double alpha_lo = 0.0;
        double alpha_hi = std::numeric_limits<double>::max();

        std::unordered_map<double, std::pair<double, bool>> cache;
        std::unordered_map<double, std::vector<double>> grad_cache; 

        bool line_search_success = false;
        for (int iter = 0; iter < 20; ++iter) {
            double f_new;
            bool gradient_evaluated = false;
            
            if (cache.find(alpha_current) != cache.end()) {
                f_new = cache[alpha_current].first;
                gradient_evaluated = cache[alpha_current].second;
            } else {
                ...
                f_new = f(x_host);
                cache[alpha_current] = std::make_pair(f_new, false);
            }

            if (f_new <= *h_f_x + C1 * alpha_current * *h_grad_dot_d) {
                std::vector<double> gradient_new;
                
                if (gradient_evaluated && grad_cache.find(alpha_current) != grad_cache.end()) {

                    gradient_new = grad_cache[alpha_current];
                } else {
                    if (!cache.find(alpha_current)->second.second) {
                        updateSolution<<<numBlocks, threadsPerBlock, 0, line_search_stream.get()>>>(
                            d_x, d_d, d_x_temp, alpha_current, size);
                    ...
                    gradient_new = grad(x_host);
                    grad_cache[alpha_current] = gradient_new;
                    cache[alpha_current].second = true;
                }

                checkCudaError(cudaMemcpyAsync(d_g_new, gradient_new.data(), size * sizeof(double), 
                                                cudaMemcpyHostToDevice, transfer_stream.get()),
                                "Failed to copy new gradient to d_g_new");
                checkCudaError(cudaStreamSynchronize(transfer_stream.get()), 
                                "Failed to synchronize transfer stream");

                double gradient_new_dot_d;
                checkCublasError(cublasDdot(cublasHandle, size, d_g_new, 1, d_d, 1, &gradient_new_dot_d),
                                "CUBLAS ddot for new gradient dot direction failed");

                if (gradient_new_dot_d >= C2 * *h_grad_dot_d) {
            ...
            if (alpha_hi < std::numeric_limits<double>::max()) {
                alpha_current = (alpha_lo + alpha_hi) / 2.0;
            } else {
                alpha_current = 2.0 * alpha_lo;
            }
            
            if (alpha_current < BACKTRACKING_TOL) {
                alpha_current = BACKTRACKING_TOL;
                ...
                f_new = f(x_host);
                std::vector<double> gradient_new = grad(x_host);

                checkCudaError(cudaMemcpyAsync(d_g_new, gradient_new.data(), size * sizeof(double), 
                                                cudaMemcpyHostToDevice, transfer_stream.get()),
                                "Failed to copy final gradient to d_g_new");
                checkCudaError(cudaStreamSynchronize(transfer_stream.get()), 
                                "Failed to synchronize transfer stream");
                break;
            }
        ...
        if (!line_search_success && alpha_current < BACKTRACKING_TOL) {
        ...
        if (!line_search_success || !cache[alpha_current].second) {
