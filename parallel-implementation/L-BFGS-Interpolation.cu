...
class CudaStream {
    ...
    vector<double> LBFGS_CUDA(
        ...
        double *h_f_x, *h_f_new, *h_grad_dot_d;
            ...
            *h_grad_dot_d = grad_dot_d;
            ...
            for (int iter = 0; iter < 20; ++iter) {
                ...
                *h_f_new = f(x_host);
                ...
                if (alpha_prev > 0) {
                    double delta_alpha = alpha_current - alpha_prev;
                    if (fabs(delta_alpha) < 1e-10) {
                        alpha_current *= 0.5;  
                    } else {
                        double grad_alpha = (*h_f_new - *h_f_x - *h_grad_dot_d * alpha_current)/
                                           (alpha_current * alpha_current);
                        double next_alpha = cubicInterpolate(alpha_prev, alpha_current, f_prev, *h_grad_dot_d, 
                                                           *h_f_new, grad_alpha);

                        if (next_alpha < 0.1 * alpha_prev || next_alpha > 0.9 * alpha_prev) {
                            next_alpha = alpha_prev * 0.5; 
                        }
                        alpha_current = next_alpha;
                    }
                } else {
                    double next_alpha = quadraticInterpolate(alpha_current, 0.0, *h_f_new, *h_grad_dot_d, *h_f_x);

                    if (next_alpha < 0.1 * INITIAL_STEP_SIZE || next_alpha > 0.9 * INITIAL_STEP_SIZE) {
                        next_alpha = INITIAL_STEP_SIZE * 0.5;
                    }
                    alpha_current = next_alpha;
                }
